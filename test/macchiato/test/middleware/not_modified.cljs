(ns macchiato.test.middleware.not-modified
  (:require [macchiato.middleware.not-modified :refer [not-modified-response wrap-not-modified]]
...
(deftest test-wrap-not-modified
  (with-redefs [macchiato.middleware.not-modified/not-modified-response #(vector %1 %2)]
               (let [req     (modified-request "Sun, 23 Sep 2012 11:00:00 GMT")
                     handler (handler-modified "Jan, 23 Sep 2012 11:00:00 GMT")]
                 (is (= [(handler req identity nil) req]               ; Not modified since is called with expected args
                        ((mock-handler wrap-not-modified handler) req identity nil))))))
